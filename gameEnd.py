...
def game_end():
    menu.clear()
    menu.add.label(Content.end.value, font_size=Display.title_fontsize.value,
    ...
    for rank in rank_list:
        if rank[1] == user:
            current_rank = rank[0]
    print(current_rank)
    # rank DB 연결 필요 # 추후 수정  ## Defs.py에 저장
    menu.add.label('Rank: %d' % current_rank)
...
menu = pygame_menu.Menu(
    '', size[Utilization.x.value], size[Utilization.y.value], theme=mytheme)
